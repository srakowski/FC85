...
void disk_Read(byte *disk, const char *fileName, byte *target, size_t sizeof_target) {
  DiskMap *dmap = (DiskMap *)disk;
  DiskFile *file = NULL;
  DiskFile *curr = disk_GetFile(disk, true);
  while (file == NULL && curr != NULL) {
    if (strncmp(curr->name, fileName, sizeof(curr->name)) == 0) {
      file = curr;
    }
  }
  assert(file);
  assert(sizeof_target >= file->size);
  memset(target, 0, sizeof_target);
  int bytesRemaining = file->size;
  byte *targetPtr = target;
  for (byte b = 0; b < file->blockCount; b++) {
    int bytesToCopy = min(bytesRemaining, DISK_BLOCK_SIZE);
    memcpy(targetPtr, dmap->blocks[file->blocks[b]], bytesToCopy);
    targetPtr += bytesToCopy;
...
static void system_Tick(System *sys) {
  ...
  if (!sys->shutdown) {
    assert((sys->count-1) < arraylen(sys->moduleStack));
...
static void machine_Tick(Machine *m) {
  ...
  SDL_SetRenderDrawColor(m->renderer, 0, 0, 0, SDL_ALPHA_OPAQUE);
...
static void sysComp_Exe(Component *, System *);
...
int main(int argc, char **argv) {
  ...
  Module *sysModule = module_Create(false);
  module_AddComponent(sysModule, component_Create("FC-85:SYS", sysComp_Exe), component_Destroy);
  system_PushModule(&_machine->sys, sysModule, module_Destroy);
...
static void sysComp_Exe(Component *cmp, System *sys) {
  static void shutdownGameMenuOption_OnPick(MenuItem *item, System *sys);
  Menu *menu = (Menu *)cmp->data;
  if (!menu->initialized) {
    menu->count = 1;
    strcpy(menu->items[0].name, "Shutdown");
    menu->items[0].onPick = shutdownGameMenuOption_OnPick;
...
static void playGameComp_Exe(Component *cmp, System *sys) {
  ...
  while (fd != NULL)
  {
    menu->items[0].onPick = playMenuOption_OnPick;
    char buffer[256] = {'\0'};
...
static void playMenuOption_OnPick(MenuItem *item, System *sys) {
  ...
  if (item->tag[0] != '\0') {
    Game *game = (Game *)sys->fswap;
    if (strncmp(item->tag, game->assets.name, min(sizeof(game->assets.name), sizeof(item->name))) != 0) {
      memset(sys->fswap, 0, sizeof(sys->fswap));
      disk_Read(sys->disk, item->tag, sys->fswap, sizeof(sys->fswap));
    }
  }
